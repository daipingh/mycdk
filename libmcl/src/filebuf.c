...
int mcl_fgets_init(mcl_fgets_t *contex, void *fp, mcl_fread_cb fread)
{
	contex->fp = fp;
	contex->fread = fread;
	contex->buf = 0;
...
int mcl_fgets_setbuf(mcl_fgets_t *contex, char *buf, size_t buf_size)
{
	if (contex->buf != NULL)
		return UV_EBUSY;

	contex->buf = buf;
	contex->buf_size = buf_size;

	return 0;
}
int mcl_fgets_feof(mcl_fgets_t *contex)
{
	if (contex->buf == NULL) {
		contex->buf = contex->inline_buf;
		contex->buf_size = sizeof(contex->inline_buf);
	}
	if (contex->buf_pos == contex->buf_len) {
		contex->buf_pos = 0;
		contex->buf_len = contex->fread(contex->buf, 1, contex->buf_size, contex->fp);
	}
	return contex->buf_pos == contex->buf_len;
}
int mcl_fgets_fgetc(mcl_fgets_t *contex)
{
	if (mcl_fgets_feof(contex))
		return -1;
	return contex->buf[contex->buf_pos++];
}
char *mcl_fgets(mcl_fgets_t *contex, char *buf, size_t buf_size)
{
	size_t len;
	int ch, complated = 0;
